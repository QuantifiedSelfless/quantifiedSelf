...
class UserDeauth(web.RequestHandler):
    ...
    def get(self):
        if(self.get_argument('email', None)):
            ...
            user = yield get_user_from_email(self.get_argument('email', None))
            if(user != None):
                token = str(uuid.uuid1())
                self._ioloop.add_callback(create_deauth_request, id=token, user_id=user['id'])
                link = "{0}/deauth?token={1}".format(self.application.settings['base_url'], token)
